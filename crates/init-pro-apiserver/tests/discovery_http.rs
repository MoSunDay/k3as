...
//!
//! Mirrors the `init-pro-api/tests/json_fidelity.rs` pattern: assert the wire
...
use init_pro_api::SchemaRegistry;
...
fn served_registry() -> SchemaRegistry {
    ...
    init_pro_api::initpro::register(&mut reg);
    reg
}
...
async fn api_endpoint_serves_core_versions() {
    ...
    let expected = serde_json::to_string(&init_pro_api::core_api_versions(&reg, "127.0.0.1:6443")).unwrap();

    let app = init_pro_apiserver::discovery_app(reg, "127.0.0.1:6443");
    let resp = app
    ...
    assert_eq!(body_string(resp).await, expected, "/api body must be byte-identical");
}

#[tokio::test]
async fn apis_endpoint_serves_group_list() {
    let reg = served_registry();
    let expected = serde_json::to_string(&init_pro_api::api_group_list(&reg)).unwrap();
    ...
    assert!(body.contains(r#""init-pro.io""#), "group list must include init-pro.io");
}

#[tokio::test]
async fn api_v1_serves_core_resource_list() {
    let reg = served_registry();
    let expected = serde_json::to_string(&init_pro_api::api_resource_list(&reg, "", "v1").unwrap()).unwrap();
...
async fn apis_initpro_v1_serves_resource_list() {
    let reg = served_registry();
    let expected =
        serde_json::to_string(&init_pro_api::api_resource_list(&reg, "init-pro.io", "v1").unwrap()).unwrap();

    let app = init_pro_apiserver::discovery_app(reg, "127.0.0.1:6443");
    let resp = app
        .oneshot(Request::builder().uri("/apis/init-pro.io/v1").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn unknown_group_version_returns_404() {
    ...
    let app = init_pro_apiserver::discovery_app(reg, "127.0.0.1:6443");
    let resp = app
        .oneshot(Request::builder().uri("/apis/fabricated.io/v9beta1").body(Body::empty()).unwrap())
        .await
        .unwrap();

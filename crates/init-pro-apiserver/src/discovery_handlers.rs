...
//!
//! Each handler is a thin transport wrapper over a [`init_pro_api::discovery`]
...
async fn core_v1_resources_handler(State(st): State<AppState>) -> DiscoveryList {
    match init_pro_api::api_resource_list(&st.registry, "", "v1") {
...
async fn api_resources_handler(
    ...
    match init_pro_api::api_resource_list(&st.registry, &group, &version) {
        Some(list) => DiscoveryList::Ok(Json(list)),
        None => DiscoveryList::NotFound,
    }
}
...
mod tests {
    ...
    //! transport. Byte fidelity of the bodies lives in `init-pro-api`.

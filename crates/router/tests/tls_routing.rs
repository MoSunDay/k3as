...
use router::{
    build_server_config, compile_ingress, ephemeral_listener, Balancer, CertKey, RouteTable,
...
fn boot_tls(
    ...
    let entries: Vec<(String, CertKey)> = certs
        ...
        .map(|c| {
            (
                c.host.clone(),
                CertKey::pem(c.cert_pem.clone(), c.key_pem.clone()),
            )
        })
        .collect();
    ...
    (
        addr,
        server,
        certs.iter().map(|c| c.cert_pem.clone()).collect(),
    )
}
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert!(
                ...
                "plaintext should not get a routed 200: {text}"
            );
            server.abort();
        })
        .await;
